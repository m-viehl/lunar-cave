...
export function generate_cave(full_config: ConfigType): Segment[] {
    ...
    let current_radius = random_range(config.min_radius, config.max_radius);
    let current_center = { x: 0, y: 0 };
    let currently_ccw = random() > 0.5;

    let total_arc_length = 0;
    let enclosed_angle_of_current_center = 0;
    let target_enclosed_angle_of_current_center = random_range(config.min_angle_per_center, config.max_angle_per_center);

    while (total_arc_length < config.arc_length) {
        // append new segment
        ...
        if (last !== null) {
            ...
            {
                let max_segment_angle = config.max_segment_arc_length / current_radius;
                let worst: number;
            ...
            if (switch_center) {
                ...
                target_enclosed_angle_of_current_center = random_range(config.min_angle_per_center, config.max_angle_per_center);
                // move center.
                let junction_point = {
                ...
                let new_radius = random_range(config.min_radius, config.max_radius);
                // scale vector
